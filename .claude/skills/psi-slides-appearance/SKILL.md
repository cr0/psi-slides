...
**The frame's footer fade is a mask, not a colour gradient.** `#frame::after` is flat `var(--paper)` under `mask-image: linear-gradient(to top, #000 52%, transparent)`. A gradient from the paper (an `oklch()` colour) to `transparent` is interpolated through grey by WebKit - a smoky band across every framed slide in Safari. Any fade to transparent over a colour variable should be written as a mask for the same reason. `test/frame-fade.mjs` holds it.
...
**The derived `--ink-soft` is the one that fails, and it fails invisibly.** It is `identity.ink` mixed `style.ink-soft` % (default 68) toward the paper, and it carries *every* quiet text in the deck at once: captions, marginalia, card sub-lines, recall references, the section outline, `.muted` figure labels, a `## question:` body. So the ink that passes its own check can still dim a whole deck - `#4d4d4d` is 7.96:1 itself and 3.73:1 soft - and no single slide makes that obvious, which is why it went unmeasured for so long while the accent beside it was checked from the start. `identityNotes()` measures it against the live light paper (`#f8f8f8`, AUDIENCE_CSS's own `:root --paper`, not the lighter print ground) and **solves** the first whole percent that clears 4.5:1, so the number it prints is the one to write. Reach for `style: {ink-soft: 80}` before darkening the ink: a house that sets `identity.ink` usually cannot change it, because the ink is the brand, and the share moves every quiet text together while leaving `--ink` alone.

**A `## question:` body is the exception to "quiet is fine".** `question-body: ink` (live only, emitted only when set) takes it out of `--ink-soft`. The tag serves two slides - a rhetorical question with an aside under it, where quiet is right, and a vote, where the body is the A/B/C options. On the second the dimming is wrong even at a passing ratio: the slide has dimmed the one thing the back row must read. Neither reading is the default's to pick, so it is a key.
...
**Under `offset` a figure box is a card too** (`figureCardCss()`, emitted from the same branch). The 22% tint, the 55% rule and the 78% edge are the card rules' numbers, and `test/gates/figure-cards.mjs` reads them out of `cardToneCss()` so the two cannot drift. SVG has no `box-shadow`, and **the edge is geometry, not a filter**: the compiler emits a `.dg-lift` copy of each box's outline behind it, `FIGURE_EDGE_PX` (5) figure units down and right (`opts.lift`, passed at compile time only under `offset`, and to the in-page editor as `window.PSI_DG_LIFT`). It was `filter: drop-shadow()`, which WebKit ignores on an element inside an SVG - the rule matched, the computed style reported the shadow, Safari drew nothing. The copy has its own geometry key (`--lift`, a copy of `--r` in every frame), so a box that moves on a beat takes its edge along with no runtime change; it is not a layout element, so label fit, anchors and the viewBox are untouched. `.bare`, `.clear` and `.dg-bar` hide it by a stylesheet rule, which follows a class change on a beat. `test/figure-edge.mjs` reads pixels past a box corner in Chromium and in WebKit. The heading is the first label line - every tspan from the second line start (`tspan[x]:not(:first-child)`) on goes back to the ink - and the heading rule names the tspans as well as the text, because `.tone-4` inverts its label on the tspans and would otherwise win. Excluded by selector: `.dg-bar`, `.bare`, `.clear`. `.emph` is re-stated after the tone rules so its accent outline survives the tint.
