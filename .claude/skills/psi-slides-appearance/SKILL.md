...
**The derived `--ink-soft` is the one that fails, and it fails invisibly.** It is `identity.ink` mixed `style.ink-soft` % (default 68) toward the paper, and it carries *every* quiet text in the deck at once: captions, marginalia, card sub-lines, recall references, the section outline, `.muted` figure labels, a `## question:` body. So the ink that passes its own check can still dim a whole deck - `#4d4d4d` is 7.96:1 itself and 3.73:1 soft - and no single slide makes that obvious, which is why it went unmeasured for so long while the accent beside it was checked from the start. `identityNotes()` measures it against the light paper and names the step. Reach for `style: {ink-soft: 80}` before darkening the ink: a house that sets `identity.ink` usually cannot change it, because the ink is the brand, and the share moves every quiet text together while leaving `--ink` alone.
